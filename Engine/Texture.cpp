...
Rectf Texture::GetSrcRect(int frame, int nrCols, int nrRows) const
{
	const int	col{ frame % nrCols },
		row{ frame / nrCols + 1 };

	Rectf srcRect{};
	srcRect.width = m_Width / nrCols;
	srcRect.height = m_Height / nrRows;
	srcRect.left = srcRect.width * col;
	srcRect.bottom = srcRect.height * row;
