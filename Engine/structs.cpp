...
bool operator==(const Point2f& p1, const Point2f& p2)
{
	const float episolon{ 0.5f };
	if (abs(p1.x - p2.x) > episolon)
		return false;
	if (abs(p1.y - p2.y) > episolon)
		return false;
	return true;
}
...
Rectf& Rectf::operator=(const Point2f& bottomLeft)
{
	left = bottomLeft.x;
	bottom = bottomLeft.y;
	return *this;
...
{
	out << "Rectf: (Left: " << r.left << ", Bottom: " << r.bottom 
